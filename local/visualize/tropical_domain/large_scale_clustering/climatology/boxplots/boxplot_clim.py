...
def plot_ax_title2(fig, ax, text):
    ax_position = ax.get_position()
    ax.text(ax_position.x0 - 0.5,                                                                                              # x-start
...
def plot_ax_title3(fig, ax, text):
    ax_position = ax.get_position()
    ax.text(ax_position.x0 + (0.25 + 0.5) / 2 - 0.05,                                                                                              # x-start
            ax_position.y0 - 0.0475,                                                                                              # y-start
            text,                        
            # fontsize = 7,  
            transform=fig.transFigure,
            ha='center', va='center'
...
def main():
    ...
    x_tfreq,   x_group,   x_name,    x_var, x_label,   x_units =    'daily',    'precip',      'precip_prctiles',   'precip_prctiles_95',               r'pr$_{95}$',   r'mm day$^{-1}$'  

    # x_tfreq,   x_group,   x_name,    x_var, x_label,   x_units =    'monthly',    'precip',      'precip_timeseries', 'precip_timeseries_mean',           r'pr$_{mean}$', r'mm day$^{-1}$'  
    ...
    for model in cL.get_model_letters():
        ...
        x_cmip.append(x.mean(dim = 'time').data)
        # print(x_cmip)
        # exit()
        model_list.append(model)    
    # [print(f) for f in x_cmip]
    data = x_cmip
...
if __name__ == '__main__':    
    # ds = xr.open_dataset('/Users/cbla0002/Desktop/work/metrics/models/IFS/precip/precip_prctiles/IFS_9_FESOM_5/precip_prctiles_IFS_9_FESOM_5_daily_0-360_-30-30_128x64_2025-01_2049-12.nc')
    # print(ds)
    # da = ds['precip_prctiles_95']
    # print(da.mean(dim = 'time').data)
    # exit()
    main()
