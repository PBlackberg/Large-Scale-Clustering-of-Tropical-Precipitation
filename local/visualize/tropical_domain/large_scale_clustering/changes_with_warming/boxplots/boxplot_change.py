...
def main():
    ...
    for model in cL.get_model_letters():
        ...
        # print(x_warm.mean(dim = 'time'))
        # print(x.mean(dim = 'time'))
        # exit()
        # dx = x_warm.std(dim = 'time')          - x.std(dim = 'time')
    ...
    # y_cmip_example = x_cmip[model_list.index('MIROC6')]
    # print(y_cmip_example)
    # exit()
    ...
    text = f'a) b) c) d) e) f) g) h) i) j)' #rf'd{x_label} [{x_units} K$^{{-1}}$]'
    # text = rf'a)'
    # text = rf'b)'
    # text = rf'{x_label}'
