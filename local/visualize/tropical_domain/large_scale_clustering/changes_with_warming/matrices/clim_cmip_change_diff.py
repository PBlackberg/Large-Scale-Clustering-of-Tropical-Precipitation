...
def plot(plot_path):
    ...
    for model in cL.get_model_letters():
        # -- historical --
        ds_x, ds_y, ds_ecs, ds_tas = xr.Dataset(),  xr.Dataset(), xr.Dataset(), xr.Dataset()
    ...
    for i, (model, ax) in enumerate(zip(models_sorted, axes_list)):
        ...
        ds_map['var'], ds_contour['var'] = (ds_y_warm[x2_label].sel(model = model) - ds_y_clim[x2_label].sel(model = model)) / (ds_tas_warm[tas_label].sel(model = model) - ds_tas_clim[tas_label].sel(model = model)), ds_y_clim[x2_label].sel(model = model)
        ...
        per_k = r'K$^{-1}$'
        label = f'{cL.get_model_letters()[model]}: {model}, {metric_text}: {ds_x_change[x1_label].sel(model = model).data:.2e} {units_text}'
        ...
        'cbar_label': f'd{x2_label} {x2_units[:-1]} {per_k}]', 'cbar_fontsize': 5, 'cbar_numsize': 5, 'cbar_label_pad': 0.0775,                                            # colorbar: label                
