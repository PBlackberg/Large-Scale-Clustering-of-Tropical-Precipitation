...
def plot_ax_title(fig, ax, text):
    ax_position = ax.get_position()
    ax.text(ax_position.x0 - 0.25,                                                                                                  # x-start
...
def plot(y, y_p, 
    ...
    # -- boxplot lines --
    y_orig = y
    y_sig = np.array([corr for corr, p in zip(y, y_p) if p < 0.05])    
    ...
    if y_highres_p is not None:
        ...
        text = ax.text(x_position, y_position,                                                                                                              # data
                        '\u25C6',                                                                                                                                # star                                   
                        color = 'w', ha='center', va='center',                                                                                              #
                        fontsize = 7, fontweight='bold', alpha = alpha)   
        text.set_path_effects([PathEffects.withStroke(linewidth = 1, foreground = color)])                                                            # background effects
...
def main():
    ...
    # x1_tfreq,   x1_group,   x1_name,    x1_var, x1_label,   x1_units =  'daily',      'doc_metrics',      'mean_area',            'mean_area_thres_precip_prctiles_90',       r'A$_m$',       r'km$^2$'    
    x1_tfreq,   x1_group,   x1_name,    x1_var, x1_label,   x1_units =  'daily',      'doc_metrics',      'mean_area',            'mean_area_thres_precip_prctiles_95',       r'A$_m$',       r'km$^2$'    
    # x1_tfreq,   x1_group,   x1_name,    x1_var, x1_label,   x1_units =  'daily',      'doc_metrics',      'mean_area',            'mean_area_thres_precip_prctiles_97',       r'A$_m$',       r'km$^2$'    
    ...
    y = get_metric2(data_type_group, data_type, dataset, y_tfreq, y_group, y_name, lon_area, lat_area, res, time_period, y_var)
...
if __name__ == '__main__':    
    ...
    # print(ds)
    # print(ds['area_fraction_thres_precip_prctiles_90'])
    # exit()
    main()
