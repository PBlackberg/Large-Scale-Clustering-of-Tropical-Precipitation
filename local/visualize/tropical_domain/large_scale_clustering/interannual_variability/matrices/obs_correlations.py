...
pF = import_relative_module('util_plot.get_subplot.corr_table_subplot_newnew',                  'utils')
...
def get_metric2(data_type_group, data_type, dataset, t_freq, metric_group, metric_name, lon_area, lat_area, resolution, time_period, metric_var):
    ...
    doc_metric = cA.detrend_month_anom(doc_metric) 
    doc_metric = doc_metric.assign_coords(time=np.arange(300))   

    return doc_metric


def get_metric3(data_type_group, data_type, dataset, t_freq, metric_group, metric_name, lon_area, lat_area, resolution, time_period, metric_var):
    folder_work, folder_scratch, SU_project, storage_project, data_projects = mS.get_user_specs(show = False)                        # user settings
    # -- specify metric --
    data_tyoe_group =   'observations'
    data_type =         dataset
    ...
    # print(doc_metric['ta_timeseries'])
    # print(doc_metric['ta_timeseries'].isel(time = 0).data)
    # print(doc_metric['ta_timeseries'].resample(time='1MS').mean().load())
    # exit()
    
    doc_metric = doc_metric.pad(time=(0, 300 - doc_metric.sizes['time']), constant_values=np.nan)   
    doc_metric = doc_metric.drop_vars("level")
    doc_metric = doc_metric.assign_coords(time=np.arange(300))     
    # print(doc_metric)
    # exit()
...
def main():
    ...
    # -- specify metrics --
    # -- DOC --    
    x1_tfreq,   x1_group,   x1_name,    x1_var,     x1_label,   x1_units =              'daily',    'doc_metrics',      'area_fraction',        'area_fraction_thres_precip_prctiles_95',                           r'A$_f$',           r'[km$^2$]'   
    ...
    # -- spatial preference --
    x4_tfreq,   x4_group,   x4_name,    x4_var,     x4_label,   x4_units =              'daily',    'doc_metrics',      'reference_proximity',  'reference_proximity_meridional_line_thres_precip_prctiles_95',     r'C$_z$',           r'[km]'  
    x5_tfreq,   x5_group,   x5_name,    x5_var,     x5_label,   x5_units =              'daily',    'doc_metrics',      'reference_proximity',  'reference_proximity_zonal_line_thres_precip_prctiles_95',          r'C$_m$',           r'[km]'  
    # x6_tfreq,   x6_group,   x6_name,    x6_var,     x6_label,   x6_units =              'daily',    'doc_metrics',      'reference_proximity',  'reference_proximity_eq_hydro_thres_precip_prctiles_95',            r'C$_{heq}$',       r'[km]'   
    ...
    x8_1_tfreq, x8_1_group, x8_1_name,  x8_1_var,   x8_1_label, x8_1_units =            'monthly',  'ta',               'ta_timeseries',        'ta_timeseries',                                                    r'T$_{500hpa}$',    r'[K]'    
    x8_2_tfreq, x8_2_group, x8_2_name,  x8_2_var,   x8_2_label, x8_2_units =            'monthly',  'slp',              'slp_gradients',        'slp_gradients_SOI',                                                r'SOI',             r'[]'    
    ...
    ds[f'{x1_label}'] =      get_metric2(data_type_group, data_type, dataset, x1_tfreq,   x1_group,   x1_name,    lon_area, lat_area, res, p_id, x1_var)
    ds[f'{x2_label}'] =      get_metric2(data_type_group, data_type, dataset, x2_tfreq,   x2_group,   x2_name,    lon_area, lat_area, res, p_id, x2_var)
    ...
    ds[f'{x4_label}'] =      - get_metric(data_type_group, data_type, dataset, x4_tfreq,   x4_group,   x4_name,    lon_area, lat_area, res, p_id, x4_var)
    ds[f'{x5_label}'] =      - get_metric(data_type_group, data_type, dataset, x5_tfreq,   x5_group,   x5_name,    lon_area, lat_area, res, p_id, x5_var)
    ...
    ds[f'{x7_label}'] =      get_metric(data_type_group, data_type, dataset, x7_tfreq,   x7_group,   x7_name,    lon_area, lat_area, res, p_id, x7_var)
    ds[f'{x8_label}'] =      get_metric(data_type_group, data_type, dataset, x8_tfreq,   x8_group,   x8_name,    lon_area, lat_area, res, p_id, x8_var)

    data_type_group, data_type, dataset = 'observations', 'ERA5', 'ERA5'
    ds[f'{x8_1_label}'] =  get_metric3(data_type_group, data_type, dataset, x8_1_tfreq,   x8_1_group,   x8_1_name,    lon_area, lat_area, res, p_id, x8_1_var)

    ds[f'{x8_2_label}'] =  get_metric(data_type_group, data_type, dataset, x8_2_tfreq,   x8_2_group,   x8_2_name,    lon_area, lat_area, res, p_id, x8_2_var)


    data_type_group, data_type, dataset = 'observations', 'CERES', 'CERES'
    ds[f'{x9_label}'] =      get_metric(data_type_group, data_type, dataset, x9_tfreq,   x9_group,   x9_name,    lon_area, lat_area, res, p_id, x9_var)
    

    data_type_group, data_type, dataset = 'observations', 'ISCCP', 'ISCCP'
    ds[f'{x10_label}'] =    get_metric(data_type_group, data_type, dataset, x10_tfreq,  x10_group,  x10_name,   lon_area, lat_area, res, p_id, x10_var)

    data_type_group, data_type, dataset = 'observations', 'ERA5', 'ERA5'
    ds[f'{x11_label}'] =    get_metric(data_type_group, data_type, dataset, x11_tfreq,  x11_group,  x11_name,   lon_area, lat_area, res, p_id, x11_var)
    
    data_type_group, data_type, dataset = 'observations', 'ISCCP', 'ISCCP'
    ds[f'{x12_label}'] =    get_metric(data_type_group, data_type, dataset, x12_tfreq,  x12_group,  x12_name,   lon_area, lat_area, res, p_id, x12_var)
...
#  {x14_units}
    ...
    data_type_group, data_type, dataset = 'observations', 'ERA5', 'ERA5'
    ds[f'{x14_label}'] =    get_metric(data_type_group, data_type, dataset, x14_tfreq,  x14_group,  x14_name,   lon_area, lat_area, res, p_id, x14_var)
    ...
    ds.attrs.update({
        ...
        'cbar_numsize': 8, 'cbar_label_pad': 0.01, 'cbar_label': r'r(metric$_{row}$, metric$_{col}$)', 'cbar_fontsize': 8,
...
if __name__ == '__main__':
    # path = '/Users/cbla0002/Desktop/work/metrics/observations/ERA5/slp/slp_gradients/ERA5/slp_gradients_ERA5_monthly_0-360_-30-30_128x64_1998-01_2022-12.nc'
    ...
    # '/Users/cbla0002/Desktop/work/metrics/observations/ISCCP/clouds/cloud_timeseries/ISCCP/cloud_timeseries_ISCCP_monthly_0-360_-30-30_128x64_1998-01_2022-12.nc'
    # '/Users/cbla0002/Desktop/work/metrics/observations/ISCCP/clouds/clouds_timeseries/ISCCP/clouds_timeseries_ISCCP_monthly_0-360_-30-30_128x64_1998-01_2022-12.nc'
