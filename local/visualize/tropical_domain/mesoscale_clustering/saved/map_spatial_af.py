...
def main():
    ...
    # -- z-metric --
    # z_tfreq,  z_group,    z_name,     z_var,  z_label,    z_units =   'daily',    'satfrac',               'satfrac_regress',                         'satfrac_regress',                              r'RH',      r'%'   
    ...
    for ii, lon_area in enumerate(lon_areas_x):
        ...
        if standardized:
            metric_name_coeffs =   f'satfrac_regress_{x_name}_regress_standard_{lon_area}'
        else:
            metric_name_coeffs =   f'satfrac_regress_{x_name}_regress_{lon_area}'
        ...
        if standardized:
            metric_name_pvalues =   f'satfrac_regress_{x_name}_pvalues_standard_{lon_area}'
        else:
            metric_name_pvalues =   f'satfrac_regress_{x_name}_pvalues_{lon_area}'
        ...
        for i, (b, p) in enumerate(zip(regression_coeffs.beta, map_pvalues.beta)):
            ...
            folder = f'{folder_scratch}/{Path(__file__).parents[0].name}/{Path(__file__).stem}/x_{x_name}_z_{z_name}'
