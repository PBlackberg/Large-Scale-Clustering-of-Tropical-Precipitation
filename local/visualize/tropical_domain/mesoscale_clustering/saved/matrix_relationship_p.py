        ...
        for j in range(num_metrics):
            if np.all(np.isnan(data_array[i, :])) or np.all(np.isnan(data_array[j, :])):
                ...
                if len(xy_list[0]) < 2 or np.isnan(xy_list[0]).all() or np.isnan(xy_list[1]).all():
                    ...
                    try:
                        y = mlR.get_linear_model_components(xy_list[:-1], xy_list[-1], show = False, standardized = True)[1]
                    except:
                        y = [np.nan, np.nan]
                correlation_matrix[i, j] = y[1]
...
def main():
    ...
    for idx, (lon_area, label) in enumerate(zip(lon_areas, labels)):
        ...
        resolution = 1.    
        lat_area = '-30:30'
        # z_tfreq,   z_group,   z_name,    z_var, z_label,   z_units =    'daily',    'satfrac',  'satfrac_timeseries',   'satfrac_timeseries',                                                           r'CRH',              r'%'    
