...
def plot(map_sig, regression_coeff, da_c, idx_rect = 0): #, da_c):
...
def main():
    ...
    # -- z-metric --
    z_tfreq,  z_group,    z_name,     z_var,  z_label,    z_units =   'daily',    'satfrac',               'satfrac_regress2',                         'satfrac_regress2',                              r'RH',      r'%'   
    # z_tfreq,  z_group,    z_name,     z_var,  z_label,    z_units =   'daily',    'ta',               'ta_mlr',                         'ta_mlr',                              r'Ta500hpa',      r'K'   
    ...
    for ii, lon_area in enumerate(lon_areas_x):
        ...
        if standardized:
            metric_name_coeffs =   f'satfrac_regress2_{x_name}_{y_name}_regress_standard_{lon_area}'
        else:
            metric_name_coeffs =   f'satfrac_regress2_{x_name}_{y_name}_regress_{lon_area}'
        ...
        if standardized:
            metric_name_pvalues =   f'satfrac_regress2_{x_name}_{y_name}_pvalues_standard_{lon_area}'
        else:
            metric_name_pvalues =   f'satfrac_regress2_{x_name}_{y_name}_pvalues_{lon_area}'
        ...
        for i, (b, p) in enumerate(zip(regression_coeffs.beta, map_pvalues.beta)):
            ...
            fig, ax, cbar_ax = plot(map_sig, regression_coeff, da_c, idx_rect = ii) #, da_c)

            # -- save figure --
            folder_work, folder_scratch, SU_project, storage_project, data_projects = mS.get_user_specs(show = False)
            folder = f'{folder_scratch}/{Path(__file__).parents[0].name}/{Path(__file__).stem}/x_{x_name}_y_{y_name}_z_{z_name}'
