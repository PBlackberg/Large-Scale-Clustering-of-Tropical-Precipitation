...
in both interannual variability and under warming in future climate projections.\add[\firstAuthorEdit]{
Clustering is quantified using the top 5\% heaviest daily precipitation instances, 
and interpreted as increasing with the area covered by heavy precipitation ($C$, temporally clustered rainfall) 
and proximity of heavily rainy points to each other and to key zonal and meridional reference lines ($P$, orthogonal to coverage).}\change[\firstAuthorEdit]{Both 
observations and global climate models indicate that large-scale clustering is sensitive to the SST gradient in the Pacific, being largest during El Ni\~no events. 
...
but observed and modelled deseasonalized interannual variability importantly diverge in the tropical-mean relative humidity and low cloudiness associated with spatially clustered states.
% (changes with warming)
Under future warming, all models from the CMIP6 ensemble project an increase in clustering associated with a narrowing of the intertropical convergence zone
...
Notably a few model outliers drive considerable spread in tropical-mean mid-tropospheric drying (r=0.44) linked to a climatological meridional clustering of heavy rainfall, suggesting potential for future model evaluation.
}
\end{abstract}
...
% \change[\coAuthorEdit]{Text to remove}{Text to add}
