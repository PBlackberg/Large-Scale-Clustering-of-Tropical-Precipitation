...
For the GPCP observations, this threshold is 16 mm day$^{-1}$ \add[\firstAuthorEdit]{and in models the threshold range between 15-17 mm day$^{-1}$}. 
...
\add[\firstAuthorEdit]{While there are conceptual differences in what type of rainfall is represented by different percentile thresholds, 
...
and scenes where precipitation is aggregated into fewer and/or larger precipitating features (Figure \ref{spatial_preference_exampleMAPS}). 
...
and $A_m$ does not describe important spatial characteristics 
...
A number of other measures of large-scale clustering are analyzed and the interrelationship of a key subset of metrics is presented in Figures S1-2 in the supporting information.
}
...
where an increase in $A_m$ increases the general proximity of heavily precipitating points to each other, $P_g$, in that the number of distinct precipitation features (N) is reduced 
(mathematically directly related as reciprocals for the same total area coverage, $Am = C / N$).
}
...
\change[\firstAuthorEdit]{
To estimate the individual effect of $A_m$, we apply the method of Pearson partial correlation
}{
To estimate the individual effect of the general proximity of heavily precipitating points to each other (Figure 3c), $P_g$, 
analogous to the climatological change in $Am$ with warming, we apply the method of Pearson partial correlation
}
...
However, further robustness testing is applied for model-spread correlations, in which potential outliers are removed before calculating the correlations.
}
...
\begin{figure}
    ...
    The panel titles show the total area coverage of heavy precipitation, $C$, and the mean area of precipitation features, $Am$, with
