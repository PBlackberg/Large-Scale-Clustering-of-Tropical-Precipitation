\section{\change[\firstAuthorEdit]{SST Drivers of Heavy Rainfall Clustering}{Mechanisms associated with Heavy Rainfall Spatial Clustering}}
This section investigates the extent to which changes in the El Ni\~no-Southern Oscillation can explain changes in clustering 
\change{across timescales.}{
, whether top of the atmosphere radiative fluxes can be argued to positively feedback on the highly clustered states, 
and if proximity measures defined in section 2 are related to the area of ascent.
}

We use \add[\firstAuthorEdit]{the Southern Oscillation Index (SOI) \citep{Ropelewski1987}} 
and the Oceanic Ni\~no Index \citep[ONI;][]{ONI_ref} to identify the state of the El Ni\~no-Southern Oscillation in interannual variability. 
\add[\firstAuthorEdit]{The SOI represents the standardized deseasonalized monthly anomalies of the surface pressure difference 
between Tahiti (17.6$^\circ{}$S, 149.6$^\circ{}$W) and Darwin (12.5$^\circ{}$S, 130.9$^\circ{}$E), 
calculated here as the three-month rolling average, and t
}
\remove[\firstAuthorEdit]{T}he ONI represents the three-month rolling average SST anomaly 
in the Ni\~no3.4 region (5$^\circ{}$S- 5$^\circ{}$N, 120$^\circ{}$-170$^\circ{}$W), 
...
are associated with El Ni\~no-like conditions. 
Firstly, SST regressed onto $A_m$ for interannual variability shows a pattern strongly reminiscent of an El Ni\~no SST signature \citep{Michael2002} 
(Figure \ref{mechanisms_obsMAP}). 
...
Finally, independent of changes in $A_f$, \add[\firstAuthorEdit]{SOI} and ONI show\remove[\firstAuthorEdit]{s} 
a \change[\firstAuthorEdit]{positive}{negative (positive)} partial correlation with $A_m$ 
(Figure \ref{mechanisms_obsSCATTER}a \add[\firstAuthorEdit]{and Figure S1 in supporting information}). 

Most CMIP models capture \remove[\firstAuthorEdit]{the observed connection between ONI and $A_m$ (Figure S1 in the supporting information) and } 
the \add[\firstAuthorEdit]{observed} independent contribution of ONI on $A_m$ outside the influence of $A_f$, as does the high-resolution GCM 
(Figure S3 in the supporting information and Figure \ref{mechanisms_obsSCATTER}b). 
...
during El Ni\~no than La Ni\~na. This is despite the fact that El Ni\~no represents a weakening of the Walker circulation 
...
\begin{figure}
    ...
    \caption{NOAA-GlobalTemp surface temperature \add[\firstAuthorEdit]{(land and ocean)}, Ts, 
...
for explaining model-spread in projected changes to clustering. Consistent with expectations, the magnitude of the weakening of the East-West Pacific SST gradient 
...
\begin{figure}
    \centering
    \includegraphics{sections/result_2/figures/clim_change_scatter_Tz_with_Aa}
    \caption{Same as Figure 4a, 
    but with the change in the climatological Pacific SST gradient, $T_z$, per Kelvin tropical warming from the historical to the SSP585 scenario simulation period across the CMIP6 ensemble as explanatory variable. 
    Models are as given in the legend in Figure 4.}
