...
def get_metric(dataset, t_freq, lon_area, lat_area, resolution, time_period, years, months, r_folder, r_filename, section_range, test):    
    ...
    for i, (year, month) in enumerate(zip(years, months)):                                                                              # loop through months
        ...
        for day in np.arange(0, len(da_month['time'])):                                                                                 # loop through days
            ...
            data_objects = [da_day, lon_area, lat_area, dataset, years]                                                                 # give data            
            metric.append(mC.calculate_metric(data_objects))                                                                            # calculate metric
...
if __name__ == '__main__':
    if not os.environ.get("PBS_SCRIPT"):                                                                                                # when run interactively (test)
        # ds = xr.open_dataset('/g/data/k10/cb4968/metrics/models/cmip/doc_metrics/mean_area/INM-CM5-0/mean_area_INM-CM5-0_daily_0-360_-30-30_128x64_2070-01_2099-12.nc')
        # print(ds)
        # exit()
