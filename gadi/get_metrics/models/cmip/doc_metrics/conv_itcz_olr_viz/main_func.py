...
def get_metric(dataset, t_freq, lon_area, lat_area, resolution, time_period, years, months, r_folder, r_filename, section_range, test):    
    ...
    if test:                                                                                                                            # for quickly testing some timesteps
        ...
        try:
            ...
            filename = f'{r_filename}_olr.nc'
            path_olr = f'{folder}/{filename}'
            da = xr.open_dataset(path)['var']
            hus = xr.open_dataset(path_hus)['var']
            olr = xr.open_dataset(path_olr)['var']
        except:
            ...
            process_request = ['hus', dataset, 'monthly', resolution, time_period]                                                      # specific humidity
            ...
            process_request = ['rlut', dataset, 'monthly', resolution, time_period]                                                     # outgoing longwave radiation
            olr = gD.get_data(process_request, process_data_further = process_data)                                                     # get data (for test)
            time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                              # pick out the relevant section
            time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                            #
            olr = olr.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()                                                            #
            print(olr)

            os.makedirs(os.path.dirname(path), exist_ok=True)
            xr.Dataset({'var': da}).to_netcdf(path)
            xr.Dataset({'var': hus}).to_netcdf(path_hus)
            xr.Dataset({'var': olr}).to_netcdf(path_olr)
        ...
        process_request = ['rlut', dataset, 'monthly', resolution, time_period]                                                          # specify data request
        olr = gD.get_data(process_request, process_data_further = process_data)                                                          # get data
        time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                                  # pick out the relevant section
        time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                                #
        olr = olr.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()           
    ...
    data_objects = [da, lon_area, lat_area, dataset, years, hus, olr]
