...
def plot_ref_line(axes, ds_line):
    ...
    for dataset, ax in zip(list(ds_line.data_vars.keys()), axes):
        if 'lon' in ds_line.dims:
            ...
            ax.scatter(lon_values, lat_values, transform=ccrs.PlateCarree(), color = 'r', s = 2.5)
...
def plot_settings(ds, title = '', threshold = 0, vmin = None, vmax = None, cmap = 'Blues'):
    ...
    ds.attrs.update({
        ...
        'yticks_fontsize':  10, 'ylabel_pad': 0.065, 'ylabel_label': 'latitude', 'ylabel_fontsize':  10, 'yticks': np.round(np.linspace(ds.lat.min(), ds.lat.max(), 5)).astype(int),  # 'yticks':           [-20, 0, 20],
        'xticks_fontsize':  10, 'xlabel_pad': 0.15, 'xlabel_label': 'longitude', 'xlabel_fontsize':  10, 'xticks': np.round(np.linspace(ds.lon.min(), ds.lon.max(), 6)).astype(int),  # 'xticks':           [30, 90, 150, 210, 270, 330],
        # -- colorbar --
        'cbar_height': 0.035, 'cbar_pad': 0.2,'cbar_label_pad': 0.125,'cbar_label': r'Specific humidity []', 'cbar_numsize': 10, 'cbar_fontsize': 10,
        })
    return ds
...
def ds_contour_settings(ds):
    ...
    ds.attrs.update({
        # -- contour --
        'name': 'var', 'threshold': [ds["var"].quantile(0.5), ds["var"].quantile(0.9)], 'color': 'k', 'linewidth': 1,        # ds["var"].quantile(0.5), ds["var"].quantile(0.75),
        })
    return ds
# ['red', 'purple', 'blue']
# fmt={levels[0]: '0.5', levels[1]: '0.75', levels[2]: '0.9'}, fmt='%1.3f'

# =========================================== main plot ===========================================
def plot(da, temp_path, lines = [], ds_ontop = None, ds_contour = None):
    # -- put data in dataset, and give specs --
    ds = xr.Dataset({'var': da})
    ds = plot_settings(ds, vmin = 0, vmax = 0.008) # * 1000, 0.01230961, 0.00736189
    ...
    if ds_ontop is None:
        ...
        contours = ax.contour(lonm, latm, ds_contour[ds_contour.attrs['name']], 
                    ...
                    colors =        ds_contour.attrs['color'], transform=ccrs.PlateCarree(),
                    linewidths =    ds_contour.attrs['linewidth'])
        ax.clabel(contours, inline = True, fontsize = 8, fmt = '%1.3f', colors = ds_contour.attrs['color']) 
