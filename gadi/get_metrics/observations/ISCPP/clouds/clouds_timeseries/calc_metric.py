...
def calculate_metric(data_objects):
    ...
    # -- check data --
    da, lon_area, lat_area, dataset, years, w500, time_period, da2 = data_objects
    da = da.sel(lon = slice(int(lon_area.split(':')[0]), int(lon_area.split(':')[1])), 
                lat = slice(int(lat_area.split(':')[0]), int(lat_area.split(':')[1]))
                )

    # -- pick out ascent and descent --
    w500 = w500.sel(lon = slice(int(lon_area.split(':')[0]), int(lon_area.split(':')[1])), 
                lat = slice(int(lat_area.split(':')[0]), int(lat_area.split(':')[1]))
                )
    ...
    ds[f'{metric_name}_low_mean'] = (da_low * da_area).sum(dim = ('lat', 'lon')) / da_area.sum()
    ...
    ds[f'{metric_name}_high_mean'] = (da_high * da_area).sum(dim = ('lat', 'lon')) / da_area.sum()
