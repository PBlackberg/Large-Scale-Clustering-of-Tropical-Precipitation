...
gD = import_relative_module('util_obs.get_era5_data_monthly',   'utils')
gD2 = import_relative_module('util_obs.get_isccp_cl_data',      'utils')
...
def get_metric(dataset, t_freq, lon_area, lat_area, resolution, time_period, years, months, r_folder, r_filename, section_range, test):    
    ...
    if test:                                                                                                                            # for quickly testing some timesteps
        ...
        try:
            ...
            process_request = ['cl_low', dataset, t_freq, resolution, time_period]                                                      # specify data request (for test)
            da = gD2.get_data(process_request, process_data_further = process_data)                                                     # get data (for test)
            ...
            process_request = ['cl_high', dataset, t_freq, resolution, time_period]                                                     # specify data request (for test)
            da2 = gD2.get_data(process_request, process_data_further = process_data)                                                    # get data (for test)
            time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                              # pick out the relevant section
            time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                            #
            da2 = da2.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()                                                            #
            print(da2)

            process_request = ['wap', 'ERA5', 'monthly', resolution, time_period, '', '']                                               # specify data request
            wap = gD.get_data(process_request, process_data_further = process_wap)                                                      # get data
            time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                              # pick out the relevant section
            time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                            #
    ...
    data_objects = [da, lon_area, lat_area, dataset, years, wap, time_period, da2]
