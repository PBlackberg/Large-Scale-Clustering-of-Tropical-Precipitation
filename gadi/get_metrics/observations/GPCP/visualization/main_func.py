...
gD = import_relative_module('util_obs.get_gpcp_data',           'utils')
gD2 = import_relative_module('util_obs.get_era5_data_monthly',  'utils')
gD3 = import_relative_module('util_obs.get_isccp_cl_data',      'utils')
...
def process_hus(da):
    da = da.sel(plev = 500e2, method='nearest')                                                                                         # mid troposphere
    return da
...
def get_metric(dataset, t_freq, lon_area, lat_area, resolution, time_period, years, months, r_folder, r_filename, section_range, test):    
    ...
    if test:                                                                                                                            # for quickly testing some timesteps
        ...
        try:
            ...
            filename = f'{r_filename}_hur.nc'
            path_hur = f'{folder}/{filename}'
            filename = f'{r_filename}_cl_low.nc'
            path_cl_low = f'{folder}/{filename}'
            da = xr.open_dataset(path)['var']
            hur = xr.open_dataset(path_hur)['var']
            cl_low = xr.open_dataset(path_cl_low)['var']
        except:
            print('no saved test data')
            print('getting data for saving ..')
            process_request = ['pr', dataset, t_freq, resolution, time_period]                                                          # precipitation
            ...
            process_request = ['hur', 'ERA5', 'monthly', resolution, time_period, '', '']                                               # relative humidity
            hur = gD2.get_data(process_request, process_data_further = process_hus)                                                     # get data
            time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                              # pick out the relevant section
            time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                            #
            hur = hur.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()                                                            #
            print(hur)

            process_request = ['cl_low', 'ISCCP', 'monthly', resolution, time_period]                                                   # clouds (low)
            cl_low = gD3.get_data(process_request, process_data_further = process_data)                                                 # get data (for test)
            time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                              # pick out the relevant section
            time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                            #
            cl_low = cl_low.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()                                                      #
            print(cl_low)

            os.makedirs(os.path.dirname(path), exist_ok=True)
            xr.Dataset({'var': da}).to_netcdf(path)
            xr.Dataset({'var': hur}).to_netcdf(path_hur)
            xr.Dataset({'var': cl_low}).to_netcdf(path_cl_low)
        ...
        process_request = ['hur', 'ERA5', 'monthly', resolution, time_period, '', '']                                                   # specify data request
        hur = gD2.get_data(process_request, process_data_further = process_hus)                                                         # get data
        time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                                  # pick out the relevant section
        time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                                #
        hur = hur.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()                                                                #

        process_request = ['cl_low', 'ISCCP', 'monthly', resolution, time_period]                                                       # specify data request
        cl_low = gD3.get_data(process_request, process_data_further = process_data)                                                        # get data
        time_str1 = f"{years[0]}-{int(months[0]):02d}"                                                                                  # pick out the relevant section
        time_str2 = f"{years[-1]}-{int(months[-1]):02d}"                                                                                #
        cl_low = cl_low.sel(time=slice(f"{time_str1}", f"{time_str2}")).load()           
    ...
    data_objects = [da, lon_area, lat_area, dataset, years, hur, cl_low]
